...
{
    public partial class GroupsSearch : LayoutsPageBase
    {
        ...
        private void bindGroups()
        {
            ...
            dataSource = ListHandler.GetGroupsInfoAll();
            RepGroupsList.DataSource = GetPagedList(dataSource, page, this.pageSize, this.keywords); ;
            RepGroupsList.DataBind();
            string pageUrl = IBUtils.CombUrlTxt(this.thisurl + "/_layouts/15/EnvisionDoc/pages/OtherManager/GroupsSearch.aspx", "keywords={0}&page={1}",
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    IBUtils.WriteCookie("groupsSearch_page_size", _pagesize.ToString(), 14400);
                    this.pageSize = _pagesize;
                }
            }
            Response.Redirect(IBUtils.CombUrlTxt(this.thisurl + "/_layouts/15/EnvisionDoc/pages/OtherManager/GroupsSearch.aspx", "keywords={0}&page={1}",
               this.keywords, "__id__"));
        }
        ...
        {
            int _pagesize;
            if (int.TryParse(IBUtils.GetCookie("groupsSearch_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
