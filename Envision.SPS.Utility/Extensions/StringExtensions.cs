...
namespace Envision.SPS.Utility.Extensions
{
    public static class StringExtensions
    {
        ...
        public static bool ToContains(this string[] aliat, string bstr)
        {
            if (!string.IsNullOrEmpty(bstr))
            {
                if (bstr.Contains('#'))
                {
                    bstr = bstr.Split('#')[1].ToString();
                }
                foreach (var aitem in aliat)
                {
                    if (aitem != "")
                    {
                        foreach (var bitem in bstr.Split('|'))
                        {

                            if (aitem.ToUpper() == bitem.ToUpper())
                            {
                                return true;
                            }
                        }
